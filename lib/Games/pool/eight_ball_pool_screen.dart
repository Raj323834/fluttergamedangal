...
class EightBallPoolScreen extends StatefulWidget {
  final String userId;

  const EightBallPoolScreen({
    Key key,
    @required this.userId,
  }) : super(key: key);

  @override
  _EightBallPoolScreenState createState() => _EightBallPoolScreenState();
}
...
class _EightBallPoolScreenState extends State<EightBallPoolScreen> {
  ...
  void asyncMethod() async {
    ...
    setState(
      () {
        ...
        usernameUpdateCount = usernameUpdateCount;
      },
    );
    bool isInstalled = await DeviceApps.isAppInstalled("com.pool");
    if (!isInstalled) {
      await appDownload();
    } else {
  ...
  poolApi(
    ...
    try {
      ...
      CommonMethods.printLog(
          StringConstants.emptyString, "Exception Occured:-> " + e.toString());
      CommonMethods.showSnackBar(
        context,
        "Server Error!",
      );
  ...
  Future<void> checkupdate(
    ...
    CommonMethods.printLog(
      ...
      "Current 8Ball version:-> " + currentVersion.toString(),
    );
    CommonMethods.printLog(
      StringConstants.emptyString,
      "Latest 8Ball version:-> " + latestVersion.toString(),
    );
  ...
  _findLocalPath() async {
    ...
    CommonMethods.printLog("Path for Downloading", directory.path.toString());
    setState(
      () {
  ...
  appDownload() async {
    ...
    await platform.invokeMethod(
      "apkDownload",
      {
        "downloadUrl": FlavorConfig.instance.variables["ballPoolDownloadUrl"],
        "gameName": GameNames.pool,
        "fileName": UrlConstants.poolInstall
      },
    );
  ...
  Future<void> openPool(var lobbyJson) async {
    CommonMethods.printLog(
      StringConstants.emptyString,
      "----------Opening 8BallPool----------",
    );
